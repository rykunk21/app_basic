...
|-----------|---------|----------|
| **Axiom Agent** | Main decision loop, manages Kanban phases | `agent-daemon.js` |
| **Webhook Server** | Receives GitHub events (PR merge, CI, issues) | `webhook-server/` |
| **Req Validator** | Validates requirement expressivity | `lib/requirement-validator.js` |
| **Test Fingerprinter** | Hashes tests to detect Green→Refactor tampering | In agent-daemon |
...
Modular validation system in `lib/requirement-validator.js`.
...
```javascript
const { addValidator } = require('./lib/requirement-validator');

addValidator('checkSecurity', (req) => {
  const hasSecurity = /auth|permission|secure/i.test(req);
  return {
    name: 'Security',
    passed: hasSecurity,
    message: hasSecurity ? 'Security considered' : 'Add security/permissions notes'
  };
});
```
...
| `red` | Ready for failing test |
| `green` | Tests passing, implement |
| `refactor` | Clean up, open PR |
| `done` | Complete |
...
|-------|---------|
| `epic` | Groups stories |
| `story` | One REQ-### work unit |
| `bug` | Defect, skips validation |
| `triage` | Bug assessment |
...
- Edit `lib/requirement-validator.js` CONFIG section
- Disable checks or adjust thresholds
...
REPO=your/repo node agent-daemon.js

# Test requirement validator
node -e "const v=require('./lib/requirement-validator'); console.log(v.validate('...'))"

# Deploy webhook changes
cd webhook-server && railway up
```
